...
def instructions_tab():
    st.markdown("""
    ## How to Use the CEQA ARGUS System

    ### Introduction
    The CEQA Watch System is a tool designed to actively monitor and track CEQA documents for early identification of projects for land acquisition teams. Included is also a ARGUS system to extract data using Azure Document Intelligence and Azure OpenAI. 
    Below are the steps to use the system, along with a detailed explanation of the processes happening behind the scenes.


    ### Step-by-Step Instructions

    #### 1. Uploading Files
    1. **Navigate to the "🧠 Process Planning Docs" tab**.
    2. **Select a Document**:
       - Choose a planning commission document from the dropdown menu.
    ...
    2. **Fetch and Filter Data**:
       - The system will automatically fetch data from PgSQL DB.
    ...
    - **Handling Long Documents**:
      - Extraction accuracy might take a hit on very long documents. In such cases, we recommend splitting the documents into smaller parts before uploading (AKA just upload public hearings).
